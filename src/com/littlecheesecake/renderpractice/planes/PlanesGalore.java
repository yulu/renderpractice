 ...
 * The slow way is creating 2000 plane object and 16 separate textures. The optimized way
 ...
 * needs to be create once, only one texture has to be uploaded, matrix transformations need
 * to be done only once on the cpu.
 */
...
public class PlanesGalore extends Object3D{
	...
	public void init(){
		...
		float[] planePositions = new float[numVertices * 3];//?why
		float[] rotationSpeeds = new float[numVertices];//?why
		...
		mPlanePositions = ByteBuffer.allocateDirect(planePositions.length 
				* Geometry3D.FLOAT_SIZE_BYTES).order(ByteOrder.nativeOrder()).asFloatBuffer();
		mPlanePositions.put(planePositions);
		
		mRotationSpeeds = ByteBuffer.allocateDirect(rotationSpeeds.length 
				* Geometry3D.FLOAT_SIZE_BYTES).order(ByteOrder.nativeOrder()).asFloatBuffer();
	...
	private void createBuffers(){
		mGeometry.createBuffer(mPlanePositionsBufferInfo, BufferType.FLOAT_BUFFER, 
				mPlanePositions, GLES20.GL_ARRAY_BUFFER);
		mGeometry.createBuffer(mRotationSpeedsBufferInfo, BufferType.FLOAT_BUFFER, 
				mRotationSpeeds, GLES20.GL_ARRAY_BUFFER);
